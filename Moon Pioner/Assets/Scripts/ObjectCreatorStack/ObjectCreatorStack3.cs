...
public class ObjectCreatorStack3 : MonoBehaviour
{
    ...
    public GameObject parentObjectStack2; // объект-родитель для полученых объектов
    public GameObject parentObjectStack3; // объект-родитель для созданных объектов
    ...
    public List<GameObject> childObjectsStack2; // список дочерних объектов которые уничтожыем
    public List<GameObject> childObjectsStack3; // список дочерних объектов которые создаем
    ...
    void CreateObject()
    {
        ...
        if (isSpawning && (childObjectsStack1.Count != 0 && childObjectsStack2.Count != 0) && childObjectsStack3.Count < maxObjects) // если достигли максимального количества объектов
        {
            ...
            GameObject lastObject1 = childObjectsStack2[childObjectsStack2.Count - 1]; // получаем последний объект из списка
            childObjectsStack2.RemoveAt(childObjectsStack2.Count - 1); // удаляем последний объект из списка
            Destroy(lastObject1); // уничтожаем объект

            GameObject newObject = Instantiate(objectPrefab, parentObjectStack3.transform.position, Quaternion.identity); // создаем новый объект
            newObject.transform.SetParent(parentObjectStack3.transform); // Делаем объект дочерним
        }
    }
    ...
    private void FillingLists()
    {
        ...
        childObjectsStack3 = new List<GameObject>();
        ...
        foreach (Transform child in parentObjectStack3.transform)
        {
           childObjectsStack3.Add(child.gameObject);
        }
    }
}
